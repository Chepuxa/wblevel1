...
В интерфейсе описываются сигнатуры методов, которые входят в контракт, и каждый тип, который реализует эти методы, реализовывает контракт.
Если в качестве параметра или возвращаемого типа мы указываем интерфейс, то можно использовать любой тип, который этот интерфейс реализует, что позволяет добиться полиморфизма.
